const Play = () => {
  return (
    <>
      {/* Breadcrumb Area Start */}
      <section className="breadcrumb-area play">
        <img className="bc-img" src="assets/images/breadcrumb/play.png" alt />
        <div className="container">
          <div className="row">
            <div className="col-lg-12">
              <h4 className="title">Play</h4>
              <ul className="breadcrumb-list">
                <li>
                  <a href="index.html">
                    <i className="fas fa-home" />
                    Home
                  </a>
                </li>
                <li>
                  <span>
                    <i className="fas fa-chevron-right" />{" "}
                  </span>
                </li>
                <li>
                  <a href="play.html">Play</a>
                </li>
              </ul>
            </div>
          </div>
        </div>
      </section>
      {/* Breadcrumb Area End */}
      {/* Play Games Area Start */}
      <section className="play-games">
        <div className="container">
          <div className="row">
            <div className="col-lg-12">
              <div className="gaming-activities">
                <div className="gaming-activities-inner">
                  <div className="row">
                    <div className="col-lg-4 col-md-6">
                      <div className="single-activities">
                        <div className="top-area">
                          <div className="left">
                            <div className="icon">
                              <img src="assets/images/play/icon1.png" alt />
                              <span>
                                4.5 <i className="fas fa-star" />
                              </span>
                            </div>
                          </div>
                          <div className="right">
                            <h4 className="title">Crypto DICE</h4>
                            <p className="text">Roll the Dice</p>
                          </div>
                        </div>
                        <div className="bottom-area">
                          <h4 className="title">People Playing:</h4>
                          <div className="players-count">
                            <ul className="players-list">
                              <li>
                                <img src="assets/images/play/people.png" alt />
                              </li>
                              <li>
                                <img src="assets/images/play/people.png" alt />
                              </li>
                              <li>
                                <img src="assets/images/play/people.png" alt />
                              </li>
                              <li>
                                <img src="assets/images/play/people.png" alt />
                              </li>
                            </ul>
                            <div className="count-num">40+</div>
                          </div>
                          <div className="pp">
                            <p>50 People Playing</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="col-lg-4 col-md-6">
                      <div className="single-activities">
                        <div className="top-area">
                          <div className="left">
                            <div className="icon">
                              <img src="assets/images/play/icon2.png" alt />
                              <span>
                                4.5 <i className="fas fa-star" />
                              </span>
                            </div>
                          </div>
                          <div className="right">
                            <h4 className="title">Crypto DICE</h4>
                            <p className="text">Roll the Dice</p>
                          </div>
                        </div>
                        <div className="bottom-area">
                          <h4 className="title">People Playing:</h4>
                          <div className="players-count">
                            <ul className="players-list">
                              <li>
                                <img src="assets/images/play/people.png" alt />
                              </li>
                              <li>
                                <img src="assets/images/play/people.png" alt />
                              </li>
                              <li>
                                <img src="assets/images/play/people.png" alt />
                              </li>
                              <li>
                                <img src="assets/images/play/people.png" alt />
                              </li>
                            </ul>
                            <div className="count-num">40+</div>
                          </div>
                          <div className="pp">
                            <p>50 People Playing</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="col-lg-4 col-md-6">
                      <div className="single-activities">
                        <div className="top-area">
                          <div className="left">
                            <div className="icon">
                              <img src="assets/images/play/icon3.png" alt />
                              <span>
                                4.5 <i className="fas fa-star" />
                              </span>
                            </div>
                          </div>
                          <div className="right">
                            <h4 className="title">Crypto DICE</h4>
                            <p className="text">Roll the Dice</p>
                          </div>
                        </div>
                        <div className="bottom-area">
                          <h4 className="title">People Playing:</h4>
                          <div className="players-count">
                            <ul className="players-list">
                              <li>
                                <img src="assets/images/play/people.png" alt />
                              </li>
                              <li>
                                <img src="assets/images/play/people.png" alt />
                              </li>
                              <li>
                                <img src="assets/images/play/people.png" alt />
                              </li>
                              <li>
                                <img src="assets/images/play/people.png" alt />
                              </li>
                            </ul>
                            <div className="count-num">40+</div>
                          </div>
                          <div className="pp">
                            <p>50 People Playing</p>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="all-games">
          <div className="container">
            <div className="row">
              <div className="col-lg-12 text-center">
                <ul className="nav" role="tablist">
                  <li className="nav-item">
                    <a
                      className="nav-link active"
                      id="pills-tab1-tab"
                      data-toggle="pill"
                      href="#pills-tab1"
                      role="tab"
                      aria-controls="pills-tab1"
                      aria-selected="true"
                    >
                      <div className="icon">
                        <img
                          className="one"
                          src="assets/images/play/ic6.png"
                          alt
                        />
                        <img
                          className="two"
                          src="assets/images/play/ic66.png"
                          alt
                        />
                      </div>
                      <span>Top Games</span>
                    </a>
                  </li>
                  <li className="nav-item">
                    <a
                      className="nav-link"
                      id="pills-tab2-tab"
                      data-toggle="pill"
                      href="#pills-tab2"
                      role="tab"
                      aria-controls="pills-tab2"
                      aria-selected="false"
                    >
                      <div className="icon">
                        <img
                          className="one"
                          src="assets/images/play/ic1.png"
                          alt
                        />
                        <img
                          className="two"
                          src="assets/images/play/ic11.png"
                          alt
                        />
                      </div>
                      <span>New</span>
                    </a>
                  </li>
                  <li className="nav-item">
                    <a
                      className="nav-link"
                      id="pills-tab3-tab"
                      data-toggle="pill"
                      href="#pills-tab3"
                      role="tab"
                      aria-controls="pills-tab3"
                      aria-selected="false"
                    >
                      <div className="icon">
                        <img
                          className="one"
                          src="assets/images/play/ic2.png"
                          alt
                        />
                        <img
                          className="two"
                          src="assets/images/play/ic22.png"
                          alt
                        />
                      </div>
                      <span>Slots</span>
                    </a>
                  </li>
                  <li className="nav-item">
                    <a
                      className="nav-link"
                      id="pills-tab4-tab"
                      data-toggle="pill"
                      href="#pills-tab4"
                      role="tab"
                      aria-controls="pills-tab4"
                      aria-selected="false"
                    >
                      <div className="icon">
                        <img
                          className="one"
                          src="assets/images/play/ic3.png"
                          alt
                        />
                        <img
                          className="two"
                          src="assets/images/play/ic33.png"
                          alt
                        />
                      </div>
                      <span>Jackpots</span>
                    </a>
                  </li>
                  <li className="nav-item">
                    <a
                      className="nav-link"
                      id="pills-tab5-tab"
                      data-toggle="pill"
                      href="#pills-tab5"
                      role="tab"
                      aria-controls="pills-tab5"
                      aria-selected="false"
                    >
                      <div className="icon">
                        <img
                          className="one"
                          src="assets/images/play/ic4.png"
                          alt
                        />
                        <img
                          className="two"
                          src="assets/images/play/ic44.png"
                          alt
                        />
                      </div>
                      <span>Video</span>
                    </a>
                  </li>
                  <li className="nav-item">
                    <a
                      className="nav-link"
                      id="pills-tab6-tab"
                      data-toggle="pill"
                      href="#pills-tab6"
                      role="tab"
                      aria-controls="pills-tab6"
                      aria-selected="false"
                    >
                      <div className="icon">
                        <img
                          className="one"
                          src="assets/images/play/ic5.png"
                          alt
                        />
                        <img
                          className="two"
                          src="assets/images/play/ic55.png"
                          alt
                        />
                      </div>
                      <span>Others</span>
                    </a>
                  </li>
                </ul>
                <div className="tab-content" id="pills-tabContent">
                  <div
                    className="tab-pane fade show active"
                    id="pills-tab1"
                    role="tabpanel"
                    aria-labelledby="pills-tab1-tab"
                  >
                    <div className="row">
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon1.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon2.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon3.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon1.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon2.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon3.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                    </div>
                    <div className="row">
                      <div className="col-lg-12 text-center">
                        <a href="#" className="mybtn1">
                          View More
                        </a>
                      </div>
                    </div>
                  </div>
                  <div
                    className="tab-pane fade"
                    id="pills-tab2"
                    role="tabpanel"
                    aria-labelledby="pills-tab2-tab"
                  >
                    <div className="row">
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon1.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon2.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon3.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon1.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon2.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon3.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                    </div>
                    <div className="row">
                      <div className="col-lg-12 text-center">
                        <a href="#" className="mybtn1">
                          View More
                        </a>
                      </div>
                    </div>
                  </div>
                  <div
                    className="tab-pane fade"
                    id="pills-tab3"
                    role="tabpanel"
                    aria-labelledby="pills-tab3-tab"
                  >
                    <div className="row">
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon1.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon2.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon3.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon1.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon2.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon3.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                    </div>
                    <div className="row">
                      <div className="col-lg-12 text-center">
                        <a href="#" className="mybtn1">
                          View More
                        </a>
                      </div>
                    </div>
                  </div>
                  <div
                    className="tab-pane fade"
                    id="pills-tab4"
                    role="tabpanel"
                    aria-labelledby="pills-tab4-tab"
                  >
                    <div className="row">
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon1.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon2.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon3.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon1.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon2.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon3.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                    </div>
                    <div className="row">
                      <div className="col-lg-12 text-center">
                        <a href="#" className="mybtn1">
                          View More
                        </a>
                      </div>
                    </div>
                  </div>
                  <div
                    className="tab-pane fade"
                    id="pills-tab5"
                    role="tabpanel"
                    aria-labelledby="pills-tab5-tab"
                  >
                    <div className="row">
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon1.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon2.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon3.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon1.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon2.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon3.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                    </div>
                    <div className="row">
                      <div className="col-lg-12 text-center">
                        <a href="#" className="mybtn1">
                          View More
                        </a>
                      </div>
                    </div>
                  </div>
                  <div
                    className="tab-pane fade"
                    id="pills-tab6"
                    role="tabpanel"
                    aria-labelledby="pills-tab6-tab"
                  >
                    <div className="row">
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon1.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon2.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon3.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon1.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon2.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                      <div className="col-lg-4 col-md-6">
                        <div className="single-game">
                          <img src="assets/images/game/icon3.png" alt />
                          <a href="#" className="mybtn2">
                            PLay NoW !
                          </a>
                        </div>
                      </div>
                    </div>
                    <div className="row">
                      <div className="col-lg-12 text-center">
                        <a href="#" className="mybtn1">
                          View More
                        </a>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
      {/* Play Games Area End */}
      {/* Recent Winners Area Start */}
      <section className="recent-winners">
        <div className="container">
          <div className="row justify-content-center">
            <div className="col-lg-8 col-md-10">
              <div className="section-heading">
                <h5 className="subtitle">Try to Check out our</h5>
                <h2 className="title">Recent Winners</h2>
                <p className="text">
                  We update our site regularly; more and more winners are added
                  every day! To locate the most recent winner's information
                </p>
              </div>
            </div>
          </div>
          <div className="row">
            <div className="col-lg-4">
              <div className="single-winer">
                <div className="top-area">
                  <div className="left">
                    <h4 className="name">Leroy Roy</h4>
                    <p className="date">01.08.2019</p>
                  </div>
                  <div className="right">
                    <p className="id">#5747e75482</p>
                  </div>
                </div>
                <div className="bottom-area">
                  <div className="left">0.099 ETH</div>
                  <div className="right">
                    <img src="assets/images/icon2.png" alt />
                  </div>
                </div>
              </div>
            </div>
            <div className="col-lg-4">
              <div className="single-winer">
                <div className="top-area">
                  <div className="left">
                    <h4 className="name">Jeff Mack</h4>
                    <p className="date">01.08.2019</p>
                  </div>
                  <div className="right">
                    <p className="id">#5747e75482</p>
                  </div>
                </div>
                <div className="bottom-area">
                  <div className="left">0.099 ETH</div>
                  <div className="right">
                    <img src="assets/images/icon2.png" alt />
                  </div>
                </div>
              </div>
            </div>
            <div className="col-lg-4">
              <div className="single-winer">
                <div className="top-area">
                  <div className="left">
                    <h4 className="name">Neal Morris</h4>
                    <p className="date">01.08.2019</p>
                  </div>
                  <div className="right">
                    <p className="id">#5747e75482</p>
                  </div>
                </div>
                <div className="bottom-area">
                  <div className="left">0.099 ETH</div>
                  <div className="right">
                    <img src="assets/images/icon2.png" alt />
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="row">
            <div className="col-lg-12 text-xl-center">
              <a className="mybtn2" href="#">
                View All{" "}
              </a>
            </div>
          </div>
        </div>
      </section>
    </>
  );
};

export default Play;
